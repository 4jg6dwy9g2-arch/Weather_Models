...
def run_ensemble_forecast(date_str, time_str, lead_time, model, num_members, scale):
    ...
    try:
        ...
        for member in range(num_members):
            ...
            result = subprocess.run(cmd_run, capture_output=True, text=True, check=True)
...
@app.route('/api/animate-spatial-data')
def api_animate_spatial_data():
    ...
    try:
        ...
        imageio.mimsave(str(temp_gif_path), frames, fps=fps)
...
if __name__ == '__main__':
    # Suppress werkzeug INFO logs (HTTP requests) to reduce console noise
    import logging
    log = logging.getLogger('werkzeug')
    log.setLevel(logging.WARNING)

    # Clean up orphaned database entries on startup
    cleanup_database()
